...
for year in years:
    df = pd.read_csv(f'informacao/informacao/dados/processado/dados_cursos_tratado_{year}.csv', sep=';')
    df['ano'] = year
    dfs.append(df)

dados_cursos_tratado = pd.concat(dfs, ignore_index=True)

dados_ies_tratado = pd.read_csv('informacao/informacao/dados/processado/dados_ies_tratado_2023.csv', sep=';')
...
# Exportando os dados
df_pivot.to_csv('informacao/informacao/dados/processado/final_ingressantes_py.csv', index=False)
